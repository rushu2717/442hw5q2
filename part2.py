...
class GridDataset(Dataset):
    ...
    def __getitem__(self,idx): # Get one image and
...
class Network(nn.Module):
    def __init__(self):
        ...
        self.base = nn.Sequential(
            nn.Conv2d(1,5,5,2,2),
            nn.ReLU(),
            nn.Conv2d(5,10,5,2,2),
            nn.ReLU(),
        )
        out_channel = 64 # the output channel number of your base module
...
optimizer = optim.Adam(model.parameters(), lr=0.005) # TODO: Experiment with different optimizer
num_epoch = 12 # TODO: Choose an appropriate number of epochs
...
#To be deleted in release
correct_image, correct_label = testset.__getitem__(2000)
correct_image = correct_image.view(1,1,56,56).to(device)
with torch.no_grad():
    output = model.visualize(correct_image)
output = output.cpu().permute(0,1,3,2).reshape(140,14).permute(1,0)
plt.imshow(correct_image.cpu().view(56,56),cmap="gray")
plt.title("Original Image, label {}".format(correct_label))
plt.show()
plt.figure(figsize=(10,100))
plt.imshow(torch.cat((output[:,:35],output[:,35:]),axis=0),cmap="gray")
plt.title("Activation map for each class")
plt.show()
